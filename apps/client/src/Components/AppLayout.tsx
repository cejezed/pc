...
export default function AppLayout({
      ...
      {NAV.map((item) => {
        ...
        return (
          <button
            ...
            className={
              active
                ? "w-full group flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-left transition-all rounded-xl bg-zeus-accent text-white shadow-[0_0_15px_rgba(255,107,0,0.4)]"
                : "w-full group flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-left transition-all text-zeus-text hover:text-white hover:bg-zeus-accent/10 rounded-xl border border-transparent hover:border-zeus-accent/30"
            }
          >
  ...
  return (
    <div className="min-h-screen text-zeus-text bg-zeus-bg">
      {/* Topbar - RESPONSIVE */}
      <header
        className="sticky top-0 z-40 flex h-12 sm:h-14 items-center justify-between px-3 sm:px-4 md:px-6 bg-zeus-card border-b border-zeus-border"
      >
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Hamburger (mobiel) */}
          <button
            aria-label="Open menu"
            className="inline-flex items-center justify-center rounded-md p-2 hover:bg-zeus-accent/10 text-zeus-accent focus-visible:outline-none focus-visible:ring-2 md:hidden"
            style={{ '--tw-ring-color': 'var(--zeus-accent)' } as React.CSSProperties}
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={onLogout}
            className="inline-flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 text-xs sm:text-sm hover:bg-zeus-accent/10 hover:text-zeus-accent hover:border-zeus-accent/50 transition-all rounded-xl border border-zeus-border text-zeus-text"
          >
        ...
        <div className="mx-auto px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-6 max-w-[1920px]">
          <div className="md:grid md:grid-cols-[220px_1fr] lg:grid-cols-[240px_1fr] xl:grid-cols-[260px_1fr] md:gap-4 lg:gap-6">
            ...
            <aside className="hidden md:block">
              <div className="sticky top-20 bg-zeus-card p-4 rounded-3xl border border-zeus-border">
                <div className="text-xs uppercase tracking-[0.2em] px-1 mb-2 text-zeus-text-highlight opacity-70 font-semibold">
                  Navigatie
                </div>
                <NavList />
                <div className="mt-6 pt-4 border-t border-zeus-border">
                  <div className="flex items-center justify-between px-1">
                    <div className="flex items-center gap-3 min-w-0">
                      <div
                        className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shrink-0 bg-zeus-card border border-zeus-accent text-zeus-accent shadow-[0_0_10px_rgba(255,107,0,0.2)]"
                      >
                      ...
                      <div className="text-xs min-w-0 flex-1">
                        ...
                        <p className="text-zeus-text truncate opacity-60">
                          {useAuth().user?.email}
                        </p>
                      </div>
                    </div>
      ...
      {mobileOpen && (
        <>
          ...
          {/* Drawer */}
          <div
            className="fixed inset-y-0 left-0 z-50 w-[280px] sm:w-72 max-w-[85%] bg-zeus-card p-4 shadow-2xl md:hidden overflow-y-auto border-r border-zeus-accent/20"
          >
