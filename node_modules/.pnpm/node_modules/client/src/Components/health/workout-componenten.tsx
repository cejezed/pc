...
export function WorkoutsTab() {
  ...
  const last7Days = useMemo(() => {
    ...
    return workouts.filter((w) => new Date(w.logged_at) >= weekAgo);
  }, [workouts]);

  const totalWorkouts = last7Days.length;

  const totalMinutes = useMemo(() => {
    return last7Days.reduce((sum, w) => sum + (w.duration_minutes ?? 0), 0);
  }, [last7Days]);

  const avgIntensity = useMemo(() => {
    const withIntensity = last7Days.filter((w) => w.intensity_level != null);
    if (!withIntensity.length) return 0;
    const sum = withIntensity.reduce((s, w) => s + (w.intensity_level ?? 0), 0);
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <form
          onSubmit={(e) => {
            ...
            onSubmit({
              workout_type: workoutType, // ⬅️ controlled i.p.v. fd.get("workout_type")
              title: String(fd.get("title") || ""),
              duration_minutes: Number(fd.get("duration_minutes")) || undefined,
              intensity_level: intensity as 1 | 2 | 3 | 4 | 5,
          ...
          className="space-y-4"
        >
          {/* (optioneel) hidden input voor traditionele FormData consumers */}
          <input type="hidden" name="workout_type" value={workoutType} />

          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <Label>Type *</Label>
              <Select value={workoutType} onValueChange={(v) => setWorkoutType(v as any)}>
                <SelectTrigger>
